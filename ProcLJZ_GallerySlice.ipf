...
Function sg_init_defaults_if_needed()
    ...
    if (!NVAR_Exists(displayMode))
        Variable/G displayMode = 0    // 0 raw, 1 d2xx, 2 d2yy, 3 d2xy
    endif
...
Function sg_pm_display_proc(ctrlName, popNumber, popText) : PopupMenuControl
    ...
    if (StringMatch(popText, "XX"))
        displayMode = 1
    elseif (StringMatch(popText, "YY"))
        displayMode = 2
    elseif (StringMatch(popText, "XY"))
        displayMode = 3
    else
        displayMode = 0
    endif
...
Window SLICEGALLERY_LJZ_P() : Panel
	...
	PopupMenu sg_pm_disp,pos={852.00,108.60},size={36.60,20.40},proc=sg_pm_display_proc
	PopupMenu sg_pm_disp,mode=1,popvalue="Raw",value= #"\"Raw;XX;YY;XY\""
...
Function/S sg_second_derivative_label()
    NVAR displayMode = root:ARPES_LJZ:SliceGallery:displayMode
...
Function/S sg_color_mode_hint_string()
    ...
    if (sg_second_derivative_view_enabled())
        return "Mode: " + sg_second_derivative_label() + " deriv + inverted CT + [auto min, 0]"
    endif
...
Function/S sg_color_range_hint_string()
    ...
    if (sg_second_derivative_view_enabled())
        if (numtype(cMin) == 0)
            sprintf outStr, "%s deriv c range: cMin≈%.4g, cMax=0", sg_second_derivative_label(), cMin
        else
            outStr = sg_second_derivative_label() + " deriv c range: cMin=auto(min), cMax=0"
        endif
        return outStr
    endif
